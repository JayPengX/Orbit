...
8. **建議**：這個 Worker 的 Settings → Placement，打開 **Smart Placement**。沒開的話 Cloudflare 預設會挑離「打這個 Worker 網址的瀏覽器」最近的邊緣節點執行，`/gemini` 對 Google 的那一次呼叫也就從那個節點的對外 IP 送出——Cloudflare 偶爾會挑到一個 Google 直接判定「地區不支援」的節點，即使使用者自己所在的地區完全合法，也會在該次請求收到 AI 匯入失敗、`AI 辨識請求失敗（400）：User location is not supported for the API use.`（見 `src/gemini-ocr.js`）。Smart Placement 改成看 Worker 自己的對外呼叫落在哪裡，讓執行位置貼近它實際在講話的對象（這裡就是 Google 的 API），而不是貼近剛好呼叫到它的瀏覽器，能明顯降低（但不保證完全消除，畢竟不是使用者自己能挑節點）這種間歇性失敗的機率。用 Wrangler CLI 部署的話這個設定已經寫在 `cloudflare-worker/wrangler.toml` 的 `[placement]` 區塊裡，不用手動到後台開。碰到這個錯誤時客戶端本身也會自動重送整輪請求（最多重試 2 次，每次都是全新的一輪，才有機會落在不同節點），失去這種節點運氣的機率因此更低；`/nl-edit` 用的是同一套重試邏輯（見下方〈AI 課表編輯（自然語言指令）〉）。**但這個重試本身不是萬能的**：Smart Placement 選的「最佳節點」是針對「這個 Worker 部署本身對 Google API 的延遲」算出來的，同一個使用者（或同一個地區的一群使用者）的請求很可能每次都落在同一個節點上，不是每次隨機挑一個——如果剛好那個節點被 Google 判定地區不支援，同一個使用者對**同一支 Worker** 的每一次重試也都會踩到同一個節點，這種情況下光是重試幾乎沒有用，症狀就是這個錯誤對某些地區的使用者變成「幾乎每次都發生」而不是偶爾發生，而且因為每一次重試都是一次真正的請求，也會加倍消耗使用者自己的每小時請求限制（見下方〈限制〉），變成「一直被擋，然後又被自己的請求限制擋住」的雙重卡關。每個回應（包含這個 400 錯誤本身）都會帶一個 `X-Worker-Colo` 標頭，值是 Cloudflare 官方的節點代碼（IATA 機場代碼，例如 `IAD`＝美國維吉尼亞）——這個代碼現在會直接顯示在錯誤訊息裡（`（節點：IAD）`這樣的字樣），回報問題時附上這個代碼，比只說「常常發生」更容易判斷是不是卡在同一個被擋的節點上。

9. **建議但非必要：第二支「備援」Worker，讓重試真的換到不同節點**。上一步的重試只在「剛好換到另一個節點」時才有用，但同一支 Worker 開了 Smart Placement 之後，Cloudflare 傾向每次都選同一個「離 Google 最近」的節點，不是每次隨機挑——對受影響的使用者來說等於每次重試都踩在同一個被擋的節點上。免費方案沒辦法指定 Worker 一定要在哪個節點執行（能做到這件事的 Durable Objects `locationHint` 需要 Cloudflare Workers Paid 方案，一個月固定費用，不是依用量計費，這個專案刻意不採用），但可以用**第二支、完全不開 Smart Placement 的 Worker** 換個角度解決：不開 Smart Placement 時 Cloudflare 用預設的「離呼叫者最近」規則選節點，跟第一支 Worker「離 Google 最近」的規則是兩套獨立邏輯，被擋時重試去打這一支，就有機會真的落在不同節點上，而不是繼續卡在原地。設定步驟：
   - 用 Wrangler CLI（`cloudflare-worker/wrangler.toml` 已經寫好 `[env.fallback]` 區塊，代管同一份 `orbit-worker.js`，只是不開 `[placement]`）：`npx wrangler deploy --env fallback` 部署，再 `npx wrangler secret put GEMINI_API_KEY --env fallback`（跟第一支用同一把金鑰即可）設定金鑰——只有這一個 Secret 需要設定，這支備援 Worker 只會被拿來重試 `/gemini`、`/nl-edit`，不會用到 `/sync` 需要的 Firebase 那兩個 Secret。用 GitHub Actions 自動部署的話，`.github/workflows/deploy-worker.yml` 已經包含這一步（`continue-on-error`，忘記設定金鑰也不會擋住主要 Worker 的部署），只需要手動跑一次上面那行 `wrangler secret put ... --env fallback`。
   - 複製這支備援 Worker 的網址，到 GitHub 專案 Settings → Secrets and variables → Actions → Variables，新增 `PROXY_URL_FALLBACK`（跟 `PROXY_URL` 同一層級的公開變數）。
   - 完全不做這一步也沒關係：`PROXY_URL_FALLBACK` 留空時，被擋的重試就退回原本「重打同一支 Worker」的行為（見上方，效果較弱但不是壞掉）。這支備援 Worker 也共用同一個 `RATE_LIMIT_KV` 計數器（見 `wrangler.toml`），不會因為多了一支 Worker 就讓使用者的每小時限制變相翻倍。

也可以用 Wrangler CLI 部署主要 Worker（見 `cloudflare-worker/wrangler.toml` 開頭註解，含 KV 命名空間的 CLI 建立指令），效果相同。想調整每小時限制次數，改 `orbit-worker.js` 裡的 `GEMINI_RATE_LIMIT`／`NL_EDIT_RATE_LIMIT` 常數即可。
...
- **不是固定動作集，是整份可編輯資料的重寫**：早期版本只認得「改某節課／搬某節課／對調兩節課／清空某節課」四種固定動作，遇到「加一個新班級」「多開一節課」「加一段下課時間」這類請求會直接沒有對應動作可用。現在的設計反過來：AI 一次讀到全部可編輯欄位（課程清單 `classes`、每天每節的排課 `weeklySchedule`、鐘聲時間 `bellTimes`、下課／午休等特殊時段 `breakTimes`、事件倒數 `countdownEvents`、單雙週對調 `reverseWeek`），回傳的也是這些欄位「套用完指令之後」的完整內容，而不是一串固定形狀的操作指令。這代表新增課程、新增/移除鐘聲節次、加一段特殊時段、加一個倒數事件、切換單雙週，都跟改一節課一樣，是同一個回傳格式自然涵蓋的情況，不需要另外設計新動作。
- **指令沒提到的部分，AI 被要求原封不動地照抄回來**：伺服器端的提示詞明確要求「只改指令有講到的地方，其餘欄位逐字照抄」，客戶端再用它跟目前資料的差異來決定套用前的預覽內容顯示什麼——差異預覽看到的永遠是「真正改了什麼」，不是整份資料。一次指令裡講好幾件事（例如「把週二第三節改物理，再加一段午休 12:00-13:00」）也是一次送出、一次回傳，AI 在同一次回覆裡把全部改動一起反映在回傳的完整狀態上。
...
- **Gemini 偶爾回報「User location is not supported」時會自動重試整輪請求**：這個錯誤是 Google 根據 Cloudflare Worker 出口 IP 判斷地區、而不是使用者實際所在地區。遇到這個錯誤時客戶端會自動重送整輪請求，最多重試 2 次，仍然失敗才會顯示錯誤——不會讓使用者自己手動重按送出。跟 AI 匯入（`gemini-ocr.js`）用的是同一套重試邏輯，包括會優先重試部署者若有設定的「備援」Worker（`PROXY_URL_FALLBACK`，見上方〈AI 辨識課表照片〉部署設定第 9 步）而不是原本那一支，因為 Smart Placement 通常會把同一個使用者的請求固定送到同一個節點，重打同一支 Worker 常常只是繼續卡在同一個被擋的節點上。
