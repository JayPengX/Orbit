...
- **只認得三種結果，其餘一律不猜**：改掉某天某節課的內容（`setSlot`：星期、節次、科目，教師/教室選填，沒講明的話優先沿用課表裡已存在的同名課程資訊，而不是留空或瞎猜）、把某天某節的課搬到另一節（`moveSlot`：只換位置，不改課程本身）、或是「看不懂/指的東西不存在」。AI 只回傳這個固定形狀的 JSON（伺服器端的 Structured Output Schema 約束），這句話本身在代理端是寫死的，客戶端永遠不會把使用者輸入的任意文字當成提示詞直接送給模型——跟 AI 匯入的安全設計完全一樣（見上方〈AI 辨識課表照片〉的〈金鑰在哪、安全性怎麼樣〉）。
...
- AI 課表編輯只認得固定的三種結果（改某節課、搬某節課、看不懂/指的東西不存在），套用前一定要看過差異預覽再確認；跟 AI 辨識一樣需要網路連線，且共用同一個每小時請求數限制。
