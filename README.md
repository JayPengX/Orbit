...
- **一次可以描述多個修改，不是只能講一件事**：一句指令最多可以拆成 8 個修改動作，依講的順序依序套用——後面的動作看得到前面動作套用後的結果（例如先把某節課搬走、再把新課排進那個剛空出來的節次，這兩件事合在一句話裡完全合理）。絕大多數指令仍然只對應一個動作，這時就跟只講一件事沒有差別。
- **只認得四種動作，其餘一律不猜**：改掉某天某節課的內容（`setSlot`：星期、節次、科目，教師/教室選填，沒講明的話優先沿用課表裡已存在的同名課程資訊，而不是留空或瞎猜）、把某天某節的課搬到另一節（`moveSlot`：只換位置，不改課程本身）、把兩個節次的內容互相對調（`swapSlot`：例如「把週一第一節跟週二第三節對調」，其中一邊是空堂也可以，效果等同搬移）、或把某節課直接清空、不排別的課進去（`clearSlot`）。指令看不懂、或指的星期／節次／課程不存在時一律回報「看不懂」或「找不到」，不會猜。AI 只回傳這個固定形狀的 JSON（伺服器端的 Structured Output Schema 約束），這句話本身在代理端是寫死的，客戶端永遠不會把使用者輸入的任意文字當成提示詞直接送給模型——跟 AI 匯入的安全設計完全一樣（見上方〈AI 辨識課表照片〉的〈金鑰在哪、安全性怎麼樣〉）。
...
- **看不懂或指稱不存在都是清楚的失敗狀態，不是當機**：AI 回傳「看不懂這個指令」時會用一句話說明原因（例如指令模糊、講的不是課表修改，或多個修改裡有一個看不懂）；回傳「指的東西不存在」時同樣會說明（例如課表根本沒有第九節、要搬移或對調的節次目前是空的）。兩者都跳出可關閉的提示視窗，不會拋出錯誤畫面，也不會有任何資料變動——多個修改動作只要有一個不合理，整句指令都不會套用，不會只套用「看起來沒問題」的那幾個。
- **伺服器回傳的內容一律視為不可信、要重新驗證**：客戶端拿到結果後會依序重新驗證每一個動作，星期/節次是否真的在範圍內、要修改的科目是否真的有填、要搬移/對調/清空的節次在「前面幾個動作套用後」是否真的有課——跟 `gemini-ocr.js` 的 `DataValidator` 對 AI 匯入結果的態度一樣，任何一項不合理就直接顯示是第幾項動作出了問題，不會把不合理的建議送進確認畫面。
...
- **AI 課表編輯**：輸入的指令文字，連同目前課表的最小必要片段（排課代碼、課程清單、鐘聲時間，見〈AI 課表編輯（自然語言指令）〉），會送到代理再轉給 Gemini。需要自己打字、按下送出才會發生。
...
- AI 課表編輯只認得固定的四種動作（改某節課、搬某節課、對調兩節課、清空某節課），一句指令最多拆成 8 個動作依序套用，看不懂/指的東西不存在一律不猜，套用前一定要看過差異預覽再確認；跟 AI 辨識一樣需要網路連線，且共用同一個每小時請求數限制。
