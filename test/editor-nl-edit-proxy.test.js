...
describe('submitNlEdit - stronger-model second opinion', () => {
  it('re-asks the stronger model when the lite model answers unclear', async () => {
    const fetchMock = vi.fn(async (url, options) => {
      const body = JSON.parse(options.body);
      if (body.model === 'gemini-3.5-flash-lite')
        return fakeNlEditResponse({ ...okState(), status: 'unclear', reason: '看不懂' });
      expect(body.model).toBe('gemini-3.7-flash');
      return fakeNlEditResponse(okChangedState());
    });
    vi.stubGlobal('fetch', fetchMock);
    const { status } = statusRecorder();
    await submitNlEdit('星期三二三節對調', { status });
    expect(fetchMock).toHaveBeenCalledTimes(2);
  ...
  it('re-asks the stronger model when the lite model proposes no actual change', async () => {
    const fetchMock = vi.fn(async (url, options) => {
      const body = JSON.parse(options.body);
      return fakeNlEditResponse(
        body.model === 'gemini-3.5-flash-lite' ? okState() : okChangedState()
      );
    });
    vi.stubGlobal('fetch', fetchMock);
    const { status } = statusRecorder();
  ...
  it('falls back to the lite answer when the stronger model call fails', async () => {
    const fetchMock = vi.fn(async (url, options) => {
      const body = JSON.parse(options.body);
      if (body.model === 'gemini-3.5-flash-lite')
        return fakeNlEditResponse({ ...okState(), status: 'unclear', reason: '看不懂' });
    ...
    await submitNlEdit('星期三二三節對調', { status });
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(calls.some(call => call.isError)).toBe(false);
    expect(document.getElementById('editor-confirm-msg').textContent).toBe('看不懂');
    clickConfirm();
    vi.unstubAllGlobals();
  });

  it('does not make a second call when the lite model already proposes a change', async () => {
